...
function SideBar() {
  ...
  const TodosNumber = () => {
    ...
    const Completed = state.todos.filter(
      (todo) => todo.attributes.ischecked
    ).length;
    const Pending = state.todos.filter(
      (todo) => !todo.attributes.ischecked
    ).length;
    const Home = state.todos.filter(
      (todo) => todo.attributes.cat === "Home"
    ).length;
    const Personal = state.todos.filter(
      (todo) => todo.attributes.cat === "Personal"
    ).length;
    const Health = state.todos.filter(
      (todo) => todo.attributes.cat === "Health"
    ).length;
    const Study = state.todos.filter(
      (todo) => todo.attributes.cat === "Study"
    ).length;
    const Other = state.todos.filter(
      (todo) => todo.attributes.cat === "Other"
    ).length;
    setCount({
      All,
      Completed,
